...
func SubBalanceKey(userID int32) string   { return fmt.Sprintf("sub_balance:%d", userID) }
...
// SyncUserBalanceCache 覆盖写入用户三池余额缓存(sub / grant / cash)。
func SyncUserBalanceCache(ctx context.Context, userID int32, subBalance, grantBalance, cashBalance int64) error {
	if RedisClient == nil {
		return fmt.Errorf("redis client is not initialized")
	}

	pipe := RedisClient.Pipeline()
	pipe.Set(ctx, SubBalanceKey(userID), subBalance, 0)
