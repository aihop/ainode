...
> 状态：**已按推荐默认值实现(2026-06-27)**。复用 `grant_balance` 作订阅赠送、新增 `sub_paid_balance`。
...
|---|---|---|---|---|
| 订阅实付 | `sub_paid_balance`(**新增**) | `sub_paid_balance:<uid>` | 1 | 剩余 → 转 `cash_balance` |
| 订阅赠送 | `grant_balance`(**复用现有**) | `grant_balance:<uid>` | 2 | 清零 |
| 充值余额 | `cash_balance`(现有) | `cash_balance:<uid>` | 3 | 不变 |

**迁移友好**:复用现有 `grant_balance` 作为「订阅赠送」(语义本就如此,数据/链路不动),只**新增 `sub_paid_balance`** 作为最高优先级池。即在现有 `grant→cash` 两级前面**插入一级 `sub_paid`**。

新增字段:
- `users.sub_paid_balance BIGINT NOT NULL DEFAULT 0`(10^8 放大,同其它金额)
- `users.sub_expires_at TIMESTAMPTZ`(本期到期时间;`grant_expires_at` 已存在,二者可合一,统一用 `sub_expires_at`)
...
```lua
-- KEYS[1]=sub_paid  KEYS[2]=grant  KEYS[3]=cash
-- ARGV[1]=cost  ARGV[2]=policy(默认 "all")
-- 返回 {status, paid_deducted, grant_deducted, cash_deducted}
...
> ⚠️ 决策#1:消费顺序 = `sub_paid → grant → cash`(已按你说的)。
...
- `diff = (p+g+c) - actual`
- `diff > 0`(预扣多了,退):按**消费逆序退回**——先退 cash,再 grant,最后 sub_paid(让可退的实付尽量留在用户手里)。
- `diff < 0`(预扣少了,补):按**消费正序补扣**(sub_paid→grant→cash),用带下限的原子补扣(同现有 `compensate.lua` 思路,扩成三池)。

退款/补扣脚本由现有 `refund.lua` / `compensate.lua` 扩成三池版本。

> ⚠️ 决策:退款逆序(cash→grant→sub_paid)是为了「尽量把可退现金留住」。如要严格「从哪扣退哪」,可改为按比例,但逆序更简单且对用户友好。
...
applySubscription(uid, newPaid, newGrant, newExpiresAt, tier):
  # —— 旧订阅收尾(固定规则)——
  movedToCash = 当前 sub_paid 剩余        # 实付剩余 → 转 cash(不没收)
  cash        += movedToCash
  ...
  写 transactions + balance_logs:
    ...
    - sub_paid_to_cash : movedToCash(实付转余额)        [movedToCash>0 时]
    - sub_paid_issue   : newPaid 发放                    [newPaid>0 时]
...
`movedToCash` 必须取 **Redis 实时 sub_paid**(非滞后 DB),否则在途消费会被重复计。建议用一个 Redis Lua 原子完成:

```lua
-- KEYS[1]=sub_paid KEYS[2]=grant KEYS[3]=cash
-- ARGV[1]=newPaid ARGV[2]=newGrant
...
```sql
-- 找出到期未续费的用户
SELECT id FROM users WHERE sub_expires_at < now() AND (sub_paid_balance>0 OR grant_balance>0)
```
...
- 所有池变动都写 `transactions` + `balance_logs`(同事务),`action_type` 区分:`sub_paid_issue` / `grant_issue` / `grant_reset` / `sub_paid_to_cash` / `usage_deduct`。
- 钱包接口 `/api/site/wallet` 增加三池明细:
  ```json
  "subPaid": .., "subPaidCents": ..,
  "grant":   .., "grantCents": ..,
  "cash":    .., "cashCents": ..,
...
1. `users` 加 `sub_paid_balance`、`sub_expires_at`(若不复用 grant_expires_at);默认 0/null。
...
5. 历史用户 `sub_paid_balance=0`,自然兼容(没有订阅实付池)。
...
1. **消费顺序** = `sub_paid → grant → cash`?(已按你说的)
2. **过期/取消时**:`sub_paid` 剩余 → cash(推荐),`grant` 清零?
3. **升级时旧赠送清零**(统一规则)?还是要保留较大赠送(破例,不建议)?
4. **APayShop 事件**能否在 订阅/升级/取消 时带上最终 `paid / grant / expiresAt / tier`?
5. **退款顺序** = 逆序 cash→grant→sub_paid(推荐)?
6. `sub_expires_at` 与现有 `grant_expires_at` 合并为一个?(推荐合并)
